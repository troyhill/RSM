...
NetCDF_Extract_new = function(
  ...
  start.ncdf = ncdf4::ncatt_get( ncdfObj, "timestamps" ) $ units
  start.ncdf = strsplit( start.ncdf, ' ' )[[1]][3] # "1965-1-1"
  start.ncdf.Date = as.Date( start.ncdf )
  
  
  cellmap               <- t( ncdf4::ncvar_get( ncdfObj, ncdfObj $ var[[ 'cellmap' ]] ) )
  ...
  if ( 'olvector' %in% tolower( variable ) || 'gwvector' %in% tolower( variable ) || 'TotalVector' %in% tolower( variable ) ) {
    ...
    ## meshNodeMap ##
    if ( ! ( 'meshNodeMap' %in% names( ncdfObj $ var ) ) ) {
      ncdf4::nc_close( ncdfObj )
      stop( 'NetCDF_Extract: meshNodeMap variable not found in ncdfObj input')
    }
    nodeMap = t( ncdf4::ncvar_get( ncdfObj, ncdfObj $ var[[ 'meshNodeMap' ]] ) )  # Map of Mesh Node ID's to mesh node indicies
    ...
    ## tricons ##
    if ( ! ( 'tricons' %in% names( ncdfObj $ var ) ) ) {
      ncdf4::nc_close( ncdfObj )
      stop( 'NetCDF_Extract: tricons variable not found in ncdfObj input')
    }
    tricons = t( ncdf4::ncvar_get( ncdfObj, ncdfObj $ var[[ 'tricons' ]] ) )      # The ID's of the nodes associated with each cell
    ...
    ## node locations ##
    if ( ! ( 'locations' %in% names( ncdfObj $ var ) ) ) {
      ncdf4::nc_close( ncdfObj )
      stop( 'NetCDF_Extract: locations variable not found in ncdfObj input')
    }
    locations = t( ncdf4::ncvar_get( ncdfObj, ncdfObj $ var[[ 'locations' ]] ) )
  ...
  if ( ! ( variable %in% names( ncdfObj $ var ) ) ) {
    ncdf4::nc_close( ncdfObj )
    stop( 'NetCDF_Extract: ', variable, ' not found in ncdfObj input')
  }
  # Get NetCDF dimension vector [5794], [5794, 14975] or [2, 5794, 14975] #
  #dimensions = ncdfObj $ var [[ variable ]] $ size
  dimensions = ncdfObj $ var [[ as.character(variable) ]] $ size
  ...
  if ( 'olvector' %in% tolower( variable ) || 'gwvector' %in% tolower( variable ) || 'totalvector' %in% tolower( variable ) ) {
    ...
    #flowData = ncvar_get( ncdfObj, as.character(variable) )
    flowData = ncvar_get( ncdfObj, ncdfObj $ var[[ variable ]], start = start, count = count )
    #flowData = ncvar_get( ncdfObj, ncdfObj $ var[[ variable ]] )
    ...
    ### streamlined file names 20200304
    ...
    location.file = 'node2Location.csv'
    ...
    varData = ncdf4::ncvar_get( ncdfObj, ncdfObj $ var[[ variable ]], start = start, count = count )
  }

...
def ReadConfigFile( configFile, verbose ):
    ...
    try:
        ...
        configParams.deleteOutput = config.get( 'DEFAULT', 'Delete_Output' )
...
def ExtractDSStoCsv( configParams, verbose ):
    ...
    try:
        try:
            # If Delete_Output
            if configParams.deleteOutput:
                try:
                    os.remove( configParams.outDir + '/' + configParams.outFile )
                except OSError, err:
            ...
            for dssPathID in configParams.dssPathIDs :
                foundID = False
                #os.system('echo "False" > /opt/physical/ML/graphics_new/fork/src/R_utils/foundID.txt')
                ...
                for dssPath in pathNameList :
                    ...
                    if dssPathID in dssPathWords and \
                       configParams.dssDataType in dssPath:
                        ...
                        foundID = True
                        #os.system('echo "True" > /opt/physical/ML/graphics_new/fork/src/R_utils/foundID.txt')
